...
if ! which eu-unstrip > /dev/null ; then
	echo
	echo "You do not have eu-unstrip (from elfutils) installed."
	echo "You will not be able to debug the Buildroot binaries as the"
	echo "debug symbols will not be saved before being stripped."
	exit 1
fi
