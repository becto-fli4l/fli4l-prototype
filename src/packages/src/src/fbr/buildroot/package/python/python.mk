...
define HOST_PYTHON_INSTALL_SOURCE
	mkdir -p $(HOST_DIR)/src
	cp -dR $(@D) $(HOST_DIR)/src/python-$(PYTHON_VERSION)
endef
HOST_PYTHON_POST_INSTALL_HOOKS += HOST_PYTHON_INSTALL_SOURCE

PYTHON_INSTALL_STAGING = YES

# libglib2 always requires libintl.so and does not adhere to --disable-nls
ifneq ($(BR2_TOOLCHAIN_HAS_FULL_GETTEXT),y)
...
# when installing to target, staging files are needed
$(PYTHON_TARGET_INSTALL_TARGET_SANDBOX) : $(PYTHON_TARGET_INSTALL_STAGING_SANDBOX)
