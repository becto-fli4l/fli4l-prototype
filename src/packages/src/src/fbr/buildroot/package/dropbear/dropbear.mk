...
define DROPBEAR_DISABLE_INETD
	$(SED) 's:\(#define INETD_MODE\).*:/*\1 */:' $(@D)/default_options.h
endef

define DROPBEAR_DISABLE_MOTD
	$(SED) 's:\(#define DO_MOTD\).*:/*\1 */:' $(@D)/default_options.h
endef

define DROPBEAR_ADAPT_FILE_PATHS
	$(SED) 's,^\(#define DSS_PRIV_FILENAME\).*,\1 "/etc/ssh/dropbear_dss_host_key",' $(@D)/default_options.h
	$(SED) 's,^\(#define RSA_PRIV_FILENAME\).*,\1 "/etc/ssh/dropbear_rsa_host_key",' $(@D)/default_options.h
	$(SED) 's,^\(#define ECDSA_PRIV_FILENAME\).*,\1 "/etc/ssh/dropbear_ecdsa_host_key",' $(@D)/default_options.h
endef
...
endef
DROPBEAR_POST_EXTRACT_HOOKS += DROPBEAR_DISABLE_INETD
else
...
DROPBEAR_POST_EXTRACT_HOOKS += DROPBEAR_ADAPT_FILE_PATHS
DROPBEAR_POST_EXTRACT_HOOKS += DROPBEAR_DISABLE_MOTD
