...
WIRELESS_TOOLS_BUILD_TARGETS = all
WIRELESS_TOOLS_INSTALL_TARGETS = install

define WIRELESS_TOOLS_INSTALL_STAGING_CMDS
	+$(TARGET_MAKE_ENV) $(MAKE) -C $(@D) CC="$(TARGET_CC)" CFLAGS="$(TARGET_CFLAGS)" \
		LDFLAGS="$(TARGET_LDFLAGS)" PREFIX="$(STAGING_DIR)" LDCONFIG=/bin/true \
		install-dynamic
...
define WIRELESS_TOOLS_BUILD_CMDS
	+$(TARGET_MAKE_ENV) $(MAKE) -C $(@D) CC="$(TARGET_CC)" CFLAGS="$(TARGET_CFLAGS)" \
		LDFLAGS="$(TARGET_LDFLAGS)" $(WIRELESS_TOOLS_BUILD_TARGETS)
endef

define WIRELESS_TOOLS_INSTALL_TARGET_CMDS
	+$(TARGET_MAKE_ENV) $(MAKE) -C $(@D) CC="$(TARGET_CC)" CFLAGS="$(TARGET_CFLAGS)" \
		LDFLAGS="$(TARGET_LDFLAGS)" PREFIX="$(TARGET_DIR)" LDCONFIG=/bin/true \
