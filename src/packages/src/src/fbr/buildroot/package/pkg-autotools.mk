...
#
# Hook to prevent inclusion of RPATHs in target binaries
#
define LIBTOOL_RPATH_HOOK
	for i in `find $($(PKG)_DIR) -name ltmain.sh`; do \
		sed -i 's/^\([[:space:]]*\*) \)\(func_append compile_rpath \|compile_rpath+=\|compile_rpath="\$$compile_rpath \).*/\1;;/' $$i; \
		sed -i "s/^\([[:space:]]*\*) \)\(\(func_append finalize_rpath \|finalize_rpath+=\|finalize_rpath=\"\$$finalize_rpath \).*\)/\1echo \$$libdir | grep -q \"^$(subst /,\/,$(subst .,\.,$(BASE_DIR)))\" || \2/" $$i; \
	done
endef
...
ifeq ($(4),target)
$(2)_PRE_CONFIGURE_HOOKS += LIBTOOL_RPATH_HOOK
endif
...
ifeq ($(4),target)
$(2)_POST_PATCH_HOOKS += LIBTOOL_RPATH_HOOK
endif
...
autotools-package = $(call inner-autotools-package,$(call pkgname,$(1)),$(call UPPERCASE,$(call pkgname,$(1))),$(call UPPERCASE,$(call pkgname,$(1))),target)
host-autotools-package = $(call inner-autotools-package,host-$(call pkgname,$(1)),$(call UPPERCASE,host-$(call pkgname,$(1))),$(call UPPERCASE,$(call pkgname,$(1))),host)
