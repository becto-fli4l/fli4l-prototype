...
PPPD_PATCH = ppp_$(PPPD_VERSION)-1+4.debian.tar.xz
PPPD_SITE = http://snapshot.debian.org/archive/debian/20180129T164727Z/pool/main/p/ppp
...
ifeq ($(BR2_PACKAGE_PPPD_MPPE),y)
PPPD_MAKE_OPTS += MPPE=y
endif
...
ifeq ($(BR2_PACKAGE_PPPD_MULTILINK),y)
...
PPPD_POST_PATCH_HOOKS += PPPD_DROP_INTERNAL_IF_PPOL2TP_H
...
define PPPD_DEBIAN_PATCHES
	tar xf $(DL_DIR)/$($(PKG)_PATCH) -C $(@D)
	$(eval $(PKG)_PATCH:=)
...
PPPD_PRE_PATCH_HOOKS += PPPD_DEBIAN_PATCHES

define PPPD_CONFIGURE_CMDS
	$(SED) 's/FILTER=y/#FILTER=y/' $(PPPD_DIR)/pppd/Makefile.linux
	$(SED) 's/USE_PAM=y/#USE_PAM=y/' $(PPPD_DIR)/pppd/Makefile.linux
...
define PPPD_INSTALL_STAGING_CMDS
	+$(MAKE) CC="$(TARGET_CC)" LD="$(TARGET_LD)" CFLAGS="$(TARGET_CFLAGS)" \
		LDFLAGS="$(TARGET_LDFLAGS)" -C $(@D) $(PPPD_MAKE_OPTS) \
		DESTDIR="$(STAGING_DIR)/usr" install-devel
endef
