...
# use always -pipe which suppresses the usage of temporary files when gcc
# talks to the assembler, which keeps the build-id stable across rebuilds;
# note that configuring UCLIBC_EXTRA_CFLAGS in uclibc.config does not work!
...
UCLIBC_MAKE_FLAGS = \
	...
	UCLIBC_EXTRA_CFLAGS="$(UCLIBC_EXTRA_CFLAGS) $(TARGET_ABI) $(TARGET_DEBUGGING)" \
	UCLIBC_EXTRA_LDFLAGS="$(UCLIBC_EXTRA_LDFLAGS) $(TARGET_DEBUGGING)" \
	HOSTCC="$(HOSTCC)"
...
define UCLIBC_KCONFIG_FIXUP_CMDS
	...
	$(call KCONFIG_SET_OPT,KERNEL_HEADERS,"$(STAGING_DIR)/usr/include",$(@D)/.config)
...
define UCLIBC_BUILD_CMDS
	+$(MAKE1) -C $(@D) $(UCLIBC_MAKE_FLAGS) headers
	+$(MAKE1) -C $(@D) $(UCLIBC_MAKE_FLAGS)
	+$(MAKE) -C $(@D)/utils \
		PREFIX=$(HOST_DIR) \
...
define UCLIBC_UPDATE_DOT_CONFIG
	$(if $(KCONFIG_FILE_DIR),
		cp -f $(UCLIBC_CONFIG_FILE) $(KCONFIG_FILE_DIR)$(UCLIBC_CONFIG_FILE))
endef
UCLIBC_KCONFIG_POST_UPDATE_HOOKS += UCLIBC_UPDATE_DOT_CONFIG
...
# uClibc is rebuilt when installing, so serialize both installation stages
$(UCLIBC_TARGET_INSTALL_TARGET_SANDBOX) : $(UCLIBC_TARGET_INSTALL_STAGING)
