...
LINUX_MINOR_VERSION = $(lastword $(subst ., ,$(LINUX_VERSION)))
LINUX_MAJOR_VERSION = $(patsubst %.$(LINUX_MINOR_VERSION),%,$(LINUX_VERSION))
ifeq ($(LINUX_MINOR_VERSION),0)
LINUX_VERSION = $(LINUX_MAJOR_VERSION)
endif

LINUX_ADD2MODULESDEP = $(LINUX_PKGDIR)/add2modules.dep-$(LINUX_MAJOR_VERSION)
LINUX_ADD2MODULESALIAS = $(LINUX_PKGDIR)/add2modules.alias-$(LINUX_MAJOR_VERSION)
LINUX_IMAGE_DIR = $(BINARIES_DIR)/linux-$(LINUX_VERSION)
...
LINUX_SOURCE = linux-$(LINUX_MAJOR_VERSION).tar.xz
...
ifneq ($(LINUX_MINOR_VERSION),0)
LINUX_PATCH = $(LINUX_SITE)/patch-$(LINUX_VERSION).xz
endif
...
LINUX_PATCH_DEPENDENCIES += toolchain
# kconfig also needs flex and bison
LINUX_PATCH_DEPENDENCIES += host-flex host-bison
...
LINUX_MAKE_FLAGS = \
	...
	INSTALL_FW_PATH=$(TARGET_DIR)/lib/modules/$(LINUX_VERSION)/firmware \
...
define LINUX_APPLY_LOCAL_PATCHES
	for p in $(filter-out ftp://% http://% https://%,$(LINUX_PATCHES)) ; do \
		if test -d $(LINUX_PKGDIR)/$(LINUX_MAJOR_VERSION)/$$p ; then \
			$(APPLY_PATCHES) $(@D) $(LINUX_PKGDIR)/$(LINUX_MAJOR_VERSION)/$$p \*.patch || exit 1 ; \
		else \
...
LINUX_KCONFIG_FILE = $(call qstrip,$(BR2_LINUX_KERNEL_CUSTOM_CONFIG_FILE))-$(FBR_ARCH)-$(LINUX_MAJOR_VERSION)
endif
...
define LINUX_UPDATE_DOT_CONFIG
	$(if $(KCONFIG_FILE_DIR),
		cp -f $(LINUX_DIR)/.config $(KCONFIG_FILE_DIR)/$(LINUX_KCONFIG_FILE))
endef
LINUX_KCONFIG_POST_UPDATE_HOOKS += LINUX_UPDATE_DOT_CONFIG
...
define LINUX_BUILD_CMDS
	@for dts in $(call qstrip,$(BR2_LINUX_KERNEL_CUSTOM_DTS_PATH)); do \
		cp -f $${dts}-$(LINUX_MAJOR_VERSION) $(KERNEL_ARCH_PATH)/boot/dts/$$(basename $${dts}) ;   \
	done
...
define LINUX_INSTALL_IMAGE
	...
	$(INSTALL) -m 0644 -D $(LINUX_DIR)/System.map $(1)/System.map
endef
else
# Otherwise, just install the unique image generated by the kernel
# build process.
define LINUX_INSTALL_IMAGE
	# kristov: we always use "kernel" here instead of $(LINUX_IMAGE_NAME)
	$(INSTALL) -m 0644 -D $(LINUX_IMAGE_PATH) $(1)/kernel
	$(INSTALL) -m 0644 -D $(LINUX_DIR)/System.map $(1)/System.map
endef
endif
...
define LINUX_INSTALL_HOST_TOOLS
	...
	if grep -q "CONFIG_DTC=y" $(@D)/.config; then	\
		flock $(HOST_DIR)/bin $(INSTALL) -D -m 0755 $(@D)/scripts/dtc/dtc $(HOST_DIR)/bin/linux-dtc ;	\
		$(if $(BR2_PACKAGE_HOST_DTC),,flock $(HOST_DIR)/bin ln -sf linux-dtc $(HOST_DIR)/bin/dtc;) \
	fi
endef


define LINUX_INSTALL_IMAGES_CMDS
	$(call LINUX_INSTALL_IMAGE,$(LINUX_IMAGE_DIR))
	$(call LINUX_INSTALL_DTB,$(LINUX_IMAGE_DIR))
endef
...
linux-modules:
	if [ -f $(LINUX_DIR)/.config -a -f $(LINUX_IMAGE_DIR)/System.map ]; then \
		grep -q "^CONFIG_HAVE_UNDERSCORE_SYMBOL_PREFIX=y" $(LINUX_DIR)/.config 2>/dev/null && \
			prefix="-P _" || prefix=; \
		$(HOST_DIR)/sbin/depmod -b $(TARGET_DIR) -ae -F $(LINUX_IMAGE_DIR)/System.map $(LINUX_VERSION) $$prefix; \
		if [ -f "$(LINUX_ADD2MODULESDEP)" ]; then \
			depfile=$(TARGET_DIR)/lib/modules/$(LINUX_VERSION)/modules.dep; \
			while read module deps; \
			do \
				if grep -q "^$$module" $$depfile; then \
					sed -i "s#^$$module.*#& $$deps#" $$depfile; \
				else \
					echo "$$module $$deps" >> $$depfile; \
				fi \
			done < "$(LINUX_ADD2MODULESDEP)"; \
		fi; \
		if [ -f "$(LINUX_ADD2MODULESALIAS)" ]; then \
			cat "$(LINUX_ADD2MODULESALIAS)" >> $(TARGET_DIR)/lib/modules/$(LINUX_VERSION)/modules.alias; \
		fi \
	fi
