...
static struct gpiod_lookup_table gpio_apu2_lookup_tbl = {
	.table = {
		GPIO_LOOKUP_IDX("gpio-apu2", 0, "apu-button", 0, GPIO_ACTIVE_LOW),
		GPIO_LOOKUP_IDX("gpio-apu2", 1, "apu-led", 0, GPIO_ACTIVE_LOW),
		GPIO_LOOKUP_IDX("gpio-apu2", 2, "apu-led", 1, GPIO_ACTIVE_LOW),
		GPIO_LOOKUP_IDX("gpio-apu2", 3, "apu-led", 2, GPIO_ACTIVE_LOW),
		{ },
	},
};
...
static bool gpio_apu2_test_dmi (void)
{
	...
	if (!strcasecmp(vendor, "PCEngines"))
		bVendor = true;

	if (!strcasecmp(vendor, "PC Engines"))
		bVendor = true;

	if (!strcasecmp(product, "apu2"))
		bProduct = true;

	if (!strcasecmp(product, "PCEngines apu2"))
		bProduct = true;

	if (!strcasecmp(product, "PC Engines apu2"))
		bProduct = true;

	if (!strcasecmp(product, "APU3"))
		bProduct = true;

	if (!strcasecmp(product, "PCEngines apu3"))
		bProduct = true;

	if (!strcasecmp(product, "PC Engines apu3"))
		bProduct = true;

	if (!strcasecmp(product, "APU4"))
		bProduct = true;

	if (!strcasecmp(product, "PCEngines apu4"))
		bProduct = true;

	if (!strcasecmp(product, "PC Engines apu4"))
		bProduct = true;

	return bVendor & bProduct;
}
...
static int gpio_apu2_probe (struct platform_device *dev)
{
	...
	/* test for PC Engines APU-2 */
	if (!gpio_apu2_test_dmi()) {
		pr_err ("not PC Engines APU-2\n");
		return -ENXIO;
	}
	...
	gpiod_add_lookup_table (&gpio_apu2_lookup_tbl);
...
{
	gpiod_remove_lookup_table (&gpio_apu2_lookup_tbl);
