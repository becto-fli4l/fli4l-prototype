...
unsigned char LCD_RS=LF;
unsigned char LCD_EN=(LCD_EN1|LCD_EN2);
...
module_param(cols, int, 0400);
MODULE_PARM_DESC(cols, "Number of columns");
module_param(lines, int, 0400);
MODULE_PARM_DESC(lines, "Number of lines");
module_param(t_short, int, 0400);
module_param(t_long, int, 0400);
module_param(a_type, int, 0400);
module_param(keys, int, 0400);
MODULE_PARM_DESC(keys, "4 keys keypad connected (1 connected  0 not connected)");
MODULE_LICENSE("GPL");
...
void init_lcd(void)
{
	...
	if (((a_type == 1) || (a_type == 5)) && (LCD_LINES > 2)) {
        ...
        if (a_type == (a_type & 3))			/* a_type 0,1 and 2 use winamp-wiring, 4,5 and 6 use 'old' wiring */
	{
          LCD_RS=INIT;
	}
	sleep_delay(240);
...
char disp;
	...
	if (! access_ok (gdata, length))
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(5,0,0) */
	    return -EFAULT;

	c=0;
	for (i=0; i<length; i++) {
		do_get_user(c,data+i);
#ifdef DEBUG
		...
		if (c==27) {
			if ((i+1)<length) {
				...
				if (c=='g') {
					...
					if((i+2) < length) {
                                                i+=1;
                                                do_get_user(new_dcol,(char *)(data+i));
                                                i+=1;
                                                do_get_user(new_dline,(char *)(data+i)); 
					dcol=new_dcol;
					dline=new_dline;
					/* Find out which LCD-Controller is meant and set LCD_EN */
					disp = (char) (dline / LCD_LINES);
                                        switch (disp) {
                                          case 0:
                                            LCD_EN=( LCD_EN1 );
					    break;
				...
				if (c<8) {
					...
					if ((i+8)<length) {
						for (cnt=0; cnt<8; cnt++) {
							do_get_user(cgenbuf[cnt],(char *)(data+i+cnt+1));
						};
						wr_cgram(c,cgenbuf);
						i+=8;
					}
 ...
 */
long lcd_ioctl(struct file * file, unsigned int ioctl_num, unsigned long ioctl_param)
{
		__u8 cmd = 0;
...
void cleanup_module(void)
{
	...
	unregister_chrdev(LCD_MAJOR,"LCD_PANEL");
	printk(KERN_INFO "LCD: module cleaned up\n");
}
...
int init_module(void)
{
	...
	if (register_chrdev(LCD_MAJOR,"LCD panel",&lcd_fops)){
		...
		if (request_region(LCD_ADDRESS,3, "LCD panel")) {
