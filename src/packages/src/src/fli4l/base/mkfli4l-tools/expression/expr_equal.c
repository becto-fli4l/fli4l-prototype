 ...
 *
 *  Last Update: $Id: expr_equal.c 44152 2016-01-22 11:56:01Z kristov $
 *****************************************************************************
 */
...
#define MODULE "expression::expr_equal"
 ...
 *
 * @param expr
 *  The expression to evaluate. The caller must ensure that it is an equality
...
expr_equal_evaluate(struct expr_t const *expr,
                    struct expr_eval_context_t *context)
{
    struct expr_literal_t *result = NULL;

    struct expr_equal_t const *const eqexp
        = (struct expr_equal_t const *) expr;
...
/**
 * Prints an equality comparison expression.
 *
 * @param expr
 *  The expression to print. The caller must ensure that it is an equality
 *  comparison expression.
 * @return
 ...
 */
static char *
expr_equal_toString(struct expr_t const *expr)
{
    struct expr_equal_t const *const eqexp
        = (struct expr_equal_t const *) expr;
...
/**
 * Clones an equality comparison expression.
 *
 * @param expr
 *  The expression to clone. The caller must ensure that it is an equality
 *  comparison expression.
 * @return
 *  The cloned expression.
 */
static struct expr_t *
expr_equal_clone(struct expr_t const *expr)
{
    struct expr_equal_t const *const eqexp
        = (struct expr_equal_t const *) expr;
    return &expr_equal_create(
        expr->scope,
 ...
 *
 * @param expr1
 *  The first expression. The caller must ensure that it is an equality
 *  comparison expression.
 * @param expr2
 *  The second expression. The caller must ensure that it is an equality
 *  comparison expression.
 * @return
 *  TRUE if both expressions are equal, FALSE otherwise.
 */
static BOOL
expr_equal_equal(struct expr_t const *expr1, struct expr_t const *expr2)
{
    struct expr_equal_t const *eqexp1
        = (struct expr_equal_t const *) expr1;
    struct expr_equal_t const *eqexp2
        = (struct expr_equal_t const *) expr2;
    return expr_equal(eqexp1->arg1, eqexp2->arg1)
        && expr_equal(eqexp1->arg2, eqexp2->arg2);
}

/**
 * Frees an equality comparison expression.
 *
 * @param expr
 *  The expression to free. The caller must ensure that it is an equality
 *  comparison expression.
 */
static void
expr_equal_free(struct expr_t *expr)
{
    struct expr_equal_t const *const eqexp
        = (struct expr_equal_t const *) expr;
...
{
    struct expr_equal_t *const eqexp
        = (struct expr_equal_t *) safe_malloc(sizeof(struct expr_equal_t));
