 ...
 * measure_isdn_rates (channel_idx)	- measure isdn rates
...
#define IMOND_VERSION		"2.2.0"		/* imond version	    */
...
static Siobytes		io_bytes[ISDN_MAX_CHANNELS];
...
circuit_device_online (char * name)
{
    ...
    if (errno != ENOTCONN)
    {
	imond_syslog (LOG_INFO, "circuit_device_online (): unknown error: %s\n",
		      strerror (errno));
    }
    return 0;
}
...
find_conf_data (char * phone, int is_outgoing_connection)
{
	...
	{
	    if (! conf_data[i].is_pppoe &&
		conf_data[i].time_table[hour] == 1 &&
		find_phone (conf_data[i].outgoing_phone, phone) == TRUE)
	    {
		/* check whether we actually picked the right circuit */
		int cidx = conf_data[i].circuit_idx;
		if (!circuits[cidx].online &&
		    circuit_device_online (circuits[cidx].device_1))
		{
		    circuits[cidx].online = 1;
		    return (i);
		}
	...
	{
	    if (! conf_data[i].is_pppoe &&
		find_phone (conf_data[i].incoming_phone, phone) == TRUE)
	    {
		/* check whether we actually picked the right circuit */
		int cidx = conf_data[i].circuit_idx;
		if (!circuits[cidx].online &&
...
find_channel_idx_by_circuit_idx (int circuit_idx)
{
	...
	if (ISDN_IS_ACTIVE)
	{
		...
		if (conf_idx >= 0)
		{
		    if (CONF_CIRCUIT_IDX == circuit_idx)
		    {
			return (channel_idx);
		    }
		}
...
#ifndef TEST
/*----------------------------------------------------------------------------
 * sgets (dest, size)			    - read line from /dev/isdninfo
 *----------------------------------------------------------------------------
 */
...
read_info (void)
{
...
#else
	sscanf (ptr_idmap, "%s %n", ISDN_DRVID, &len);
	ptr_idmap += len;

	sscanf (ptr_usage, "%d %n", &usage, &len);
	ptr_usage += len;

	sscanf (ptr_phone, "%s %n", ISDN_PHONE, &len);
	ptr_phone += len;
	...
	if (!(usage & 7))
	{
	    strcpy (ISDN_STATUS, (usage & 64) ?	EXCLUSIVE_STATUS_STR :
						OFFLINE_STATUS_STR);
	...
	else
	{
	    ...
	    if (online)
	    {
		if (ISDN_ONLINE_START == 0)		/* new connection?  */
		{
		    online_counter++;
		    ...
		    strcpy (ISDN_STATUS, ONLINE_STATUS_STR);

		    gettimeofday (&(last_time_isdn), NULL);
		    ISDN_ONLINE_START    = time ((time_t *) NULL);

		    if (usage & ISDN_USAGE_OUTGOING)
		    {
			infos[channel_idx].conf_idx =
					find_conf_data (ISDN_PHONE, TRUE);
			strcpy (ISDN_INOUT, "Outgoing");
			ISDN_IS_OUTGOING_DIR = TRUE;
		    }
		    else
		    {
			infos[channel_idx].conf_idx =
					find_conf_data (ISDN_PHONE, FALSE);
			strcpy (ISDN_INOUT, "Incoming");
			ISDN_IS_OUTGOING_DIR = FALSE;
		    }
		}
	    ...
	    {
		strcpy (ISDN_STATUS, CALLING_STATUS_STR);
	    ...
	    switch (usage & 7)
	      {
	      case ISDN_USAGE_RAW:
		    strcpy (ISDN_USAGE, "Raw");
		    break;
		case ISDN_USAGE_MODEM:
		    strcpy (ISDN_USAGE, "Modem");
		    break;
		case ISDN_USAGE_NET:
		    strcpy (ISDN_USAGE, "Net");
		    break;
		case ISDN_USAGE_VOICE:
		    strcpy (ISDN_USAGE, "Voice");
		    break;
		case ISDN_USAGE_FAX:
		    strcpy (ISDN_USAGE, "Fax");
		    break;
		default:
		    strcpy (ISDN_USAGE, "-----");
		    break;
	  ...
	  if(run_updown)
	    {
	      ...
	      strcpy(devname, circuit_idx >= 0 ? circuits[circuit_idx].device_1 : "<unknown>");
	      if(0 == strncmp(devname, "isdn", 4))
		{
		  imond_syslog (LOG_INFO, "ISDN %s call from: %s on device: %s\n",
				ISDN_USAGE,
				circuit_idx >= 0 ? circuits[circuit_idx].name : "<unknown>",
				devname
				);

		  sprintf(rawup, "/etc/ppp/raw-%s %s %s",
			  updown,
			  circuit_idx >= 0 ? circuits[circuit_idx].name : "<unknown>",
			  devname
			  );
	...
	if (channel_hung_up)
	{
	    ...
	    time_t  now		= time ((time_t *) 0);
	    time_t  diff	= now - ISDN_ONLINE_START;
	    ...
	    ISDN_LAST_IBYTES        = 0;
	    ISDN_LAST_OBYTES        = 0;
	    ISDN_IBYTES_PER_SECOND  = 0;
	    ISDN_OBYTES_PER_SECOND  = 0;
	    ISDN_ONLINE_TIME	    += diff;
	    ISDN_ONLINE_START	    = 0;
	    ISDN_IP_ADDRESS[0]	    = '\0';
	}
    }
...
set_status (char * arg)
{
    ...
    *p = '\0';
	...
	else if (!strcmp (arg1, "down") || !strcmp (arg1, "restart"))
	{
	    ...
	    check_routing (); /* must set route again */
	}
    }
...
check_routing ()
{
	...
	if (strcmp (default_device, device) != 0)
	{
	    ...
	    sprintf (buf + strlen (buf),
		     "/usr/local/bin/fli4lctrl dial %s; ",
		     default_device);
	}
    }
    else
...
print_status (int client_idx, char * buf)
{
	...
	*arg = '\0';
	...
	if (! strcmp (buf, "links"))
	{
	    ...
	    if (circuit_idx >= 0 && circuit_idx < n_circuits)
	    {
		channel_idx = find_channel_idx_by_circuit_idx (circuit_idx);

		if (channel_idx >= 0 && ISDN_ONLINE_START != 0)
		{
	...
	if (! strcmp (buf, "route"))
	{
		...
		{				    /* -1: automatic rtg    */
		    ...
		    show_routing (0, "before check_routing ()");
#endif
		    check_routing ();
#ifdef DEBUG_ROUTES
		    show_routing (1, "after check_routing ()");
#endif
		    strcpy (answer, "OK\r\n");
		}
	    }
		 ...
		 ! strcmp (buf, "removelink"))
	{
		...
		{					/* only if online   */
			...
			! strcmp (buf, "addlink"))
		    {
			rtc = addlink (circuit_idx, channel_idx);

			if (rtc == 0)
			{
			    rtc = 2;		/* return remaining links   */
			}
			else
			{
		    ...
		    else if (*(circuits[circuit_idx].device_2) &&
			     circuits[circuit_idx].link_added == 1 &&
			     ! strcmp (buf, "removelink"))
		    {
			rtc = removelink (circuit_idx, channel_idx);

			if (rtc == 0)
			{
			    rtc = 1;		/* return remaining links   */
			}
			else
			{
			    rtc = 0;		/* 0: no channel bundling   */
	...
	if (! strcmp (buf, "adjust-time"))
	{
		...
		for (channel_idx = 0; channel_idx < ISDN_MAX_CHANNELS;
		     channel_idx++)
		{
		    if (ISDN_IS_ACTIVE && ISDN_ONLINE_START != 0)
		    {
			ISDN_ONLINE_START += idx;
		    }
		}

		if (last_time_isdn.tv_sec > 0)
		{
		    last_time_isdn.tv_sec += idx;
		}
 ...
 * measure_isdn_rates (channel_idx)	    - measure isdn rates
 *
 *  bandwidth_rate  bandwidth_time  result
 *	0		0	    add link immediately
 *	5		30	    add/remove link after 30 seconds
 *	-1		-1	    add/remove never
 *----------------------------------------------------------------------------
 */
static void
measure_isdn_rates (int channel_idx)
{
    if (ISDN_ONLINE_START > 0)
    {
	int conf_idx    = ISDN_CONF_IDX;
	...
	if (ioctl (isdninfo_fd, IIOCGETCPS, io_bytes) == 0)
	{
	    ...
	    ibytes = io_bytes[channel_idx].ibytes - ISDN_LAST_IBYTES;
	    obytes = io_bytes[channel_idx].obytes - ISDN_LAST_OBYTES;
	    ISDN_IBYTES += ibytes;
	    ISDN_OBYTES += obytes;
	    ISDN_LAST_IBYTES = io_bytes[channel_idx].ibytes;
	    ISDN_LAST_OBYTES = io_bytes[channel_idx].obytes;


	    ISDN_IBYTES_PER_SECOND = (int)
		((float) ibytes / seconds_passed_isdn);

	    ISDN_OBYTES_PER_SECOND = (int)
		((float) obytes / seconds_passed_isdn);
		...
		*(circuits[circuit_idx].device_2) &&
	    ...
	    {
		if (ISDN_IBYTES_PER_SECOND + ISDN_OBYTES_PER_SECOND >=
		    circuits[circuit_idx].bandwidth_rate)
		{
		    ISDN_TIME_BW_LOWER = 0;

		    if (ISDN_TIME_BW_HIGHER == 0)
		    {
			ISDN_TIME_BW_HIGHER = current_time.tv_sec;
		    }
		    else if (circuits[circuit_idx].link_added == 0 &&
			     current_time.tv_sec - ISDN_TIME_BW_HIGHER
			     >= circuits[circuit_idx].bandwidth_time)
		    {
			addlink (circuit_idx, channel_idx);
		    }
		}
		else
		{
		    ISDN_TIME_BW_HIGHER = 0;

		    if (ISDN_TIME_BW_LOWER == 0)
		    {
			ISDN_TIME_BW_LOWER = current_time.tv_sec;
		    }
		    else if (circuits[circuit_idx].link_added > 0 &&
			     current_time.tv_sec - ISDN_TIME_BW_LOWER
			     >= circuits[circuit_idx].bandwidth_time)
		    {
			removelink (circuit_idx, channel_idx);
		    }
		}
    ...
    return;
} /* measure_isdn_rates (channel_idx) */
...
main (int argc, char ** argv)
{
	...
	if (gettimeofday (&current_time, NULL) == 0)
	{
	    ...
	    if (seconds_passed_isdn >= MIN_MEASURE_INTERVAL &&
		seconds_passed_isdn < POLLING_INTERVAL + 5)
	    {					       /* sometimes 4294!   */
		for (channel_idx = 0; channel_idx < ISDN_MAX_CHANNELS;
		     channel_idx++)
		{
