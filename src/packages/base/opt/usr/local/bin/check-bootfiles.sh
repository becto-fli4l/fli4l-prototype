...
BOOT_DIR=/boot
boot_files_std='kernel rootfs.img opt.img rc.cfg'
...
then
    ...
    get_one_var arch ARCH $BOOT_DIR/rc.cfg
    case $arch in
    x86|x86_64)
        boot_files_std="$boot_files_std syslinux.cfg"
        ;;
    rpi)
        : # TBD
        ;;
    sunxi)
        boot_files_std="$boot_files_std extlinux/extlinux.conf"
        ;;
    esac
