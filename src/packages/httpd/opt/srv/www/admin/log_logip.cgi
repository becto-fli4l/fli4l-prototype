#!/bin/sh
#------------------------------------------------------------------------------
# logip.cgi - show log of wan-ip
#
# Creation:     20007-10-18 lanspezi
# Last Update:  $Id: log_logip.cgi 51847 2018-03-06 14:55:07Z kristov $
#
#------------------------------------------------------------------------------
...
if [ ! -f ${LOGIP_LOGDIR}/logip.log -o "$logip_ok" = "false" ]
then
    show_html_header "$_MN_err"
    show_error "$_MN_err" "logip.log $_MM_notexist."
    show_html_footer
    exit 1
fi
...
case $FORM_action in
    reset|$_MN_reset)
        show_html_header "$_MN_resetting ${LOGIP_LOGDIR}/logip.log ..." "refresh=$ATIME;url=$myname"
        > ${LOGIP_LOGDIR}/logip.log
        show_info "" "$_MN_resetting ${LOGIP_LOGDIR}/logip.log"
        show_html_footer
    ;;
    download|$_MN_download)
        http_header download "ctype=text/plain;filename=logip.log"
        cat ${LOGIP_LOGDIR}/logip.log
        ;;
    *)
        : ${FORM_sort:=reverse}
        ...
        case $FORM_lines in
            all|"$_MN_all"|*[^0-9]*)
                ...
                case $FORM_sort in
                    normal)
                        cat ${LOGIP_LOGDIR}/logip.log | format_output > /tmp/logip.$$
                    ;;
                    *)
                        cat ${LOGIP_LOGDIR}/logip.log | sed '1!G;h;$!d' | format_output > /tmp/logip.$$
                    ;;
                esac
            ;;
            *[0-9]*)
                ...
                case $FORM_sort in
                    normal)
                        cat ${LOGIP_LOGDIR}/logip.log | do_tail $n | format_output > /tmp/logip.$$
                    ;;
                    *)
                        cat ${LOGIP_LOGDIR}/logip.log | do_tail $n | sed '1!G;h;$!d' | format_output > /tmp/logip.$$
                    ;;
                esac
            ;;
        esac
