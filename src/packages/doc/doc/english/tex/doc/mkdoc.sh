...
create_empty_doc ()
{
    ...
    At the moment there is no \\LaTeX-based documentation available
    for this package.
...
create_error_doc ()
{
    ...
    \\section {Tex errors for package $heading}

    Although \\LaTeX-based documentation is available for this package,
    unfortunately this can not be translated due to errors. These errors
    are the following:
    \begin{verbatim}
EOF
    cat $3 >> $2
...
create_document ()
{
...

    {
        ...
        echo "\\chapter{Differences between versions}"
