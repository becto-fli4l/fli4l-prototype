...
create_empty_doc ()
{
    ...
    \\section {Paket $heading}

    Für dieses Paket steht im Augenblick noch keine \\LaTeX-basierte
...
create_error_doc ()
{
    ...
    \\section {Tex-Fehler für Paket $heading}

    Für dieses Paket steht zwar eine tex Dokumentation zur Verfügung,
    sie läßt sich jedoch leider aufgrund von Fehlern nicht
    übersetzen. Die Fehler sind folgende:
    \begin{verbatim}
EOF
    cat $3 >> $2
    cat >> $2 <<EOF
...
create_document ()
{
...

    {
        ...
        echo "\\chapter{Versionsunterschiede}"
