% Synchronized to r35810
\documentclass{book}
\usepackage[francais]{babel}
\usepackage[utf8]{inputenc}
\usepackage{makeidx}
\usepackage{graphicx}
\usepackage{longtable}
\usepackage{multirow}

\usepackage{html}

\usepackage{verbatimfiles}
\usepackage{alltt}
\usepackage{fancyvrb}
\usepackage{amssymb}
\usepackage{mathabx}

\input{version.tex}

%% Beschreibung einer Konfigurationsvariablen
%%
%% \config{name}{index eintrag}{label}{beschreibung}
%%
%% 1.Parameter = Name der Konfigurationsvariablen
%% 2.Parameter = Eintrag für den Index
%% 3.Parameter = Eintrag für entsprechendes Label, auch für hyperref
%% 4.Parameter = Beschreibung der Variablen
%% Bemerkungen: Legt auch ein Label für PDF- und HTML-Links an
%%

% Klammern einer Variablen im fließenden Text
% \var{variable}

% Einen Link erstellen, der die URL anzeigen
%
% \altlink{url}
%
% 1. Parameter: eine beliebige externe URL
%
% \link{url}
%
% 1. Parameter: eine beliebige externe URL
%
% N.B. \link ist 'deprecated'. Verwende stattdessen \altlink.

% Eine Warnung in den Text einfügen
%
% \achtung{text}
%
% 1. Parameter: Text der Warnung

% Label setzen, sowohl für Druck als auch PDF
%
% \marklabel{Name}{zugehöriger Text}
%
% 1. Parameter: Name des labels
% 2. Parameter: Text, z.B. der Anfang eines Kapitel, den man gleich
%               mit einem Label versehen möchte
% BSP: \marklabel{OPTSYSLOG}{\section{OPT\_SYSLOG - ...}}

% Link (Referenz) auf Label erzeugen
%
% \jump{Name}{Linktext}
% \smalljump{Name}{Linktext}
%
% 1. Parameter: Name eines mit marklabel erzeugten Labels
% 2. Parameter: Linktext
%
% Bsp: \jump{OPTSYSLOG}{siehe \var{OPT\_\-SYSLOG}}
%
% Anmerkung: \jump fügt beim pdf-Output zusätzlich zum Link noch die
%            Seitennummer hinzu

\renewcommand{\email}[1]{courriel~:~\htmladdnormallink {#1}{mailto:#1}}
% Ignore the following warning:
%  *** wrap_cmd_email not defined, cannot wrap \email

\newcommand{\configvar}[2]{\item[#1]#2}
\newcommand{\achtung}[1]{\medskip\textbf{#1}\vspace{1.0ex}}
\newcommand{\wichtig}[1]{\medskip\textbf{Important:} \emph{#1}\vspace{1.0ex}}
\newcommand{\config}[4]{\item[\index{#2}\label{#3}#1]#4}
\newcommand{\configlabel}[2]{\index{#1}\label{#2}}
\newcommand{\altlink}[1]{\htmladdnormallink {#1}{#1}}
\newcommand{\link}[1]{\htmladdnormallink {#1}{#1}}
\newcommand{\marklabel}[2]{#2\label{#1}}
\newcommand{\jump}[2]{\hyperref{#2}{}{}{#1}}
\newcommand{\smalljump}[2]{\hyperref{#2}{}{}{#1}}

\newcommand{\enter}{$\dlsh$}
\newcommand{\pfeil}{$\Rightarrow$}
\newcommand{\var}[1]{\begin{small}#1\end{small}}
\newcommand{\pdfbkmrk}[3]{}         % dummy
\newcommand{\flhypersetup}[1]{}     % dummy
\newcommand{\mtr}[1]{{\color{red}#1}} % mtr = _m_issing _tr_anslation
\newenvironment{example}{ \begin{small} }{ \end{small} }

% latex2html does not know the commands for french double quotes:-(
% define those commands here:
\newcommand{\flqq}{«}
\newcommand{\frqq}{»}
% fix for unneeded "unknown command" in *.aux:
\newcommand{\textlatin}{}
% workaround for not properly handled command \copyright:
\renewcommand{\copyright}{\begin{rawhtml}&copy;\end{rawhtml}}
% Ignore the following warning:
%  *** wrap_cmd_copyright not defined, cannot wrap \copyright
